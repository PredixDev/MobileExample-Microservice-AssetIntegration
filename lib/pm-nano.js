...
function init(pmConfig, myLogger) {
  return new BPromise(function(resolve, reject) {
    var pmURL = pmConfig.requireConfig('PM_UAA_URL');
    ...
    .then(function(token) {
      ...
      nano.config.url = pmURL+'/pg/api/admin/data/';
      resolve({
          nano: nano,
          pmDb: nano.db.use(bucketName)
      });
    })
    .catch(function(err) {
      logger.error(err);
...
function ensureSlashAtEnd(uri) {
    var reEndsInSlash = new RegExp("/$", "");
    if (!reEndsInSlash.test(uri)) {
        return uri + "/";
    }
    return uri;
}
