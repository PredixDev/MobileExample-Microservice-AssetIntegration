  ...
  return new Promise(function (resolve, reject) {
    ...
    function callback(error, response, body) {
      if ((!error && response.statusCode == 200)) {
        ...
        .then(function(arr){
          // logger.trace('PM service Token: ',arr);
          logger.debug("Rcvd PM service oauth-token.");
          resolve(true);
        })
        .catch(function(err){
          logger.error(err);
        ...
        logger.error({error : error, status : response.statusCode}, "Error while trying to authenticate with Asset service!");
  ...
  return new Promise(function(resolve, reject) {
    ...
    function assets_callback(error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        logger.error({error : error, status : response.statusCode, response: body}, "Error while getting Assets from URL: "+assetUrlSelected);
        reject(JSON.stringify(response));
      }
    }
